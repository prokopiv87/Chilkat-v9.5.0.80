// CkDateTimeU.h: interface for the CkDateTimeU class.
//
//////////////////////////////////////////////////////////////////////
...
#ifndef _CkDateTimeU_H
#define _CkDateTimeU_H
...
// CLASS: CkDateTimeU
class CkDateTimeU  : public CkUtf16Base
{
    private:
	
	// Don't allow assignment or copying these objects.
	CkDateTimeU(const CkDateTimeU &);
	CkDateTimeU &operator=(const CkDateTimeU &);

    public:
	CkDateTimeU(void);
	virtual ~CkDateTimeU(void);

	static CkDateTimeU *createNew(void);
	...
	const uint16_t *getAsTimestamp(bool bLocal);
	const uint16_t *asTimestamp(bool bLocal);
	
	// Sets the date/time from an RFC 3339 date/time formatted string.
	bool SetFromTimestamp(const uint16_t *rfc3339Str);
	...
	// 1/0.
	void DeSerialize(const uint16_t *serializedDateTime);
	...
	//     } 	FILETIME;
	// 
	bool GetAsFileTime(bool bLocal, FILETIME &fTime);
	...
	// bLocal indicates whether a local or UTC time is returned.
	// 
	const uint16_t *getAsRfc822(bool bLocal);
	// Returns the date/time as an RFC822 formatted string. (An RFC822 format string is
	// what is found in the "Date" header field of an email.)
	// 
	// bLocal indicates whether a local or UTC time is returned.
	// 
	const uint16_t *asRfc822(bool bLocal);
	...
	CkDtObjU *GetDtObj(bool bLocal);
	bool SetFromDtObj(CkDtObjU &dtObj);
	...
	bool SetFromRfc822(const uint16_t *rfc822Str);
	...
	bool ExpiresWithin(int n, const uint16_t *units);
	...
	bool OlderThan(int n, const uint16_t *units);


	 bool AddSeconds(int numSeconds);
	 bool GetAsUnixTimeStr(bool bLocal, CkString &outStr);
	 const uint16_t *getAsUnixTimeStr(bool bLocal);
	 bool GetAsIso8601(const uint16_t *formatStr, bool bLocal, CkString &outStr);
	 const uint16_t *getAsIso8601(const uint16_t *formatStr, bool bLocal);
